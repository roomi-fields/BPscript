...
alias_value = "cc" , ":" , INT , [ "(" , kv_pairs , ")" ]      (* cc:74, cc:1(min:0,max:100) *)
...
directive_body = IDENT                              (* @core, @controls *)
               ...
               | "alias" , IDENT , alias_value       (* @alias depart touches.z — voir alias_directive *)
...
### `alias_directive` — DÉSIGNATION : un nom, ce qu'il désigne

```ebnf
alias_directive = "@" , "alias" , IDENT , alias_value ;   (* @alias depart touches.z   @alias breath cc:2 *)
```

Décisions Romain 2026-07-27 : **le `=` est supprimé** de tout le langage, **le nom vient d'abord**
— comme toutes les autres directives nomment avant de valoriser — et la **flèche** est
**exclusivement une production**.

> Le `=` ne partait pas parce qu'il ne disait rien : il marquait **« ce nom se joue-t-il ou pas »**
> — jouable pour une macro, refusé dans le flux pour un alias. **Deux sens pour un même signe**, et
> c'est ce qui l'a condamné.
...
#### ⚠️ Un ALIAS n'est PAS une MACRO — et voici où ils divergent
...
| | **`@macro`** | **`@alias` (désignation)** |
|---|---|---|
| son nom s'écrit… | **dans la règle**, à sa place | **jamais** dans une règle |
| a un **corps** | oui | non |
| a des **paramètres** | oui | non |
| **qui le résout** | **Kairos** (`projection/projeter.ts`, `action/resoudre-macro.ts`) | **l'orchestrateur BPx** (`orchestrator/astToSceneSpec.ts`, `orchestrator.ts`) |
| **quand** | à la **projection**, **feuille par feuille** | au **chargement de la scène**, **une fois** |
| **ce qui le déclenche** | **un mot qui paraît dans le flux** | **rien** — il ne se déclenche pas, il nomme |

> **En une phrase** : une **macro** est quelque chose qu'on **écrit dans la musique** ; un
> **alias**, un **nom donné à quelque chose**. Et **brancher n'est ni l'un ni l'autre** — c'est le
> rôle des chevrons, dans le flux.

Ce qu'il faudrait pour une directive unique : donner à `@alias` un corps, des paramètres et le droit
d'être joué — à ce moment **c'est `@macro` qui l'aurait absorbé**, pas l'inverse.

...
**Signature (contrat partagé — CONVERGÉE avec Kairos, mais GELÉE jusqu'à validation Romain)** :
`(ctx: DigitalFnContext) => void` — **mutation in-place de `target`** (PAS un retour de `Partial`).
NB : nos messages s'étaient CROISÉS (Kairos avait convergé vers mon pure-return, j'avais concédé vers sa
mutation) ; on est désormais **tous deux sur la mutation** (Kairos KAI-B03 da6735d), mais l'architecte a
escaladé le CHOIX de signature à Romain ([135]) → on **ne finalise pas** tant qu'il n'a pas tranché.
Raison du fond (mutation > pure-return) :

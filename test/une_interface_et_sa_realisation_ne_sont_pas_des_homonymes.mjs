#!/usr/bin/env node
/**
 * GARDE — la clé `realises` : une INTERFACE et sa RÉALISATION ne sont pas deux homonymes.
 *
 ...
 * nom SANS `realises` restent une erreur, exactement comme avant — volet 1. Avec `realises`, la
 ...
 * paire sera déclarée.
 *
 * INJECTION dans l'ACCUSÉ (le `realises` retiré, la cible faussée) et dans le JUGE.
 */
...
{
  const { ctx } = charger(interfaceLib(), realisationLib(null));
...
// ── 1. SANS `realises`, LA RÈGLE NE CHANGE PAS — deux homonymes restent ambigus ──────────────
{
  const { ctx } = charger(interfaceLib(), realisationLib(null));
  ok(ctx.ambiguousControls.has('zzvolume'),
     "1. deux déclarations SANS `realises` doivent rester AMBIGUËS — la règle de Romain sur "
     + "l'appel nu ne bouge pas, et c'est le complément qui empêche ce garde de décrire un langage "
     + 'plus permissif que le vrai');
}

// ── 2. AVEC `realises`, IL N'Y A PLUS QU'UNE ENTRÉE PUBLIQUE ─────────────────────────────────
{
  const { ctx } = charger(interfaceLib(), realisationLib('zzface.zzvolume'));
  ok(!ctx.ambiguousControls.has('zzvolume'),
     "2a. avec `realises`, le nom ne doit PLUS être ambigu — l'interface est l'unique entrée "
  ...
  ok(ctx.realisations['zzface.zzvolume'] && ctx.realisations['zzface.zzvolume'].includes('zzmidi.zzvolume'),
     "2e. le lien interface → réalisations doit être porté par le contexte, lisible de l'aval");
  ok(ctx.realisePar['zzmidi.zzvolume'] === 'zzface.zzvolume',
     '2f. et le lien inverse aussi');
}

// ── 3. INJECTION DANS L'ACCUSÉ — un `realises` qui pointe dans le vide échoue ────────────────
{
  const { erreur } = charger(interfaceLib(), realisationLib('zzface.zzvolme'));
...
{
  const { erreur } = charger(interfaceLib(), realisationLib('zzmidi.zzvolume'));
...
{
  const { erreur } = charger(interfaceLib(), realisationLib('zzface.zzabsent'));
...
restaurer();
{
  ...
  ok(Object.keys(ctx.realisePar).length === 0,
     `5a. aucune réalisation n'est déclarée aujourd'hui — vues : ${Object.keys(ctx.realisePar).join(', ')}. `
...
const juger = (quals, realisePar) => {
  const faces = quals.filter((q) => !realisePar[q]);
  const reals = quals.filter((q) => realisePar[q]);
  return !(reals.length > 0 && faces.length === 1 && reals.every((q) => realisePar[q] === faces[0]));
};

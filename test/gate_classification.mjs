...
export const HORS_PORTILLON = new Map([
  ...
  // ⚠️ CETTE ENTRÉE ÉTAIT ÉCRITE DEUX FOIS, à deux formulations près. Dans une Map, la seconde
  // écrase la première en silence : la ligne du dessus ne servait à rien, et le compte
  // d'exclusions que le méta-garde surveille en portait une de moins qu'il n'y paraissait.
  ['formes_d_un_message.mjs', 'VERIFICATEUR des formes BPScript d un brouillon de message, lance avec un chemin AVANT d envoyer : sans argument il sort en usage(code 2), il n a rien a garder au portillon. Il ferme le trou que mes documents n avaient pas -- eux sont gardes, mes messages ne l etaient pas, et c est par la qu une graphie inventee est arrivee jusqu a Romain le 2026-07-28'],
  ['sceller_baseline_bps.mjs', 'PRODUCTEUR de la baseline de la voie .bps (consigne Romain 2026-08-11) : il INVOQUE voie_b_status.mjs, deja hors portillon pour la meme raison -- la chaine complete sur 87 scenes prend plusieurs minutes. Et son resultat est un ETAT a lire, pas un verdict : 20 productions identiques sur 81 presentes est le point de depart d un chantier, pas une regression. Branche au portillon il ajouterait vingt minutes a chaque push pour reimprimer un constat. Ce qu il scelle est garde, lui, par la_baseline_bps_dit_ses_conditions.mjs, qui est AU portillon'],
  ['corpus_de_refus.mjs', 'COMPTE d ampleur du chantier de gestion d erreurs(Romain 2026-07-28) : il RAPPORTE combien d ecritures fautives passent au lieu d etre refusees, il ne juge pas. Tant que le compte n est pas a zero, le brancher au portillon le rendrait rouge en permanence -- et un garde toujours rouge apprend a etre ignore. Il entre au portillon le jour ou le compte atteint zero, et ce jour-la il devient un garde anti-retour'],

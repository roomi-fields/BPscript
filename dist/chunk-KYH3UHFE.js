...
var MESSAGES = {
  ...
  PARSE_TEMPLATE_RANK_IS_AN_INTEGER: "the rank of a catalog entry is an INTEGER between brackets \u2014 '[3]'. Received '{p1}'. The native engine does not reject this: it reads the rank digit by digit, turns every non-digit into a zero and keeps accumulating, so '[1z]' silently becomes rank 10 and '[zzz]' becomes rank 0 \u2014 two entries can end up sharing a rank with nothing to say so.",
  PARSE_TEMPLATE_LINE_NOT_A_CATALOG_ENTRY: "in the 'template' section, every line is a catalog entry and opens with its rank in brackets \u2014 '[1] /1 ??'. This line ({p1}) opens with something else, so it belongs to no entry. It used to be DROPPED without a word, along with everything after it.",
  ...
  RESOLVE_RULE_NEVER_REACHED: "the rule '{name}' is defined but nothing invokes it \u2014 no other rule names it, and it is not the starting symbol '{axiome}'. Either it is dead, or the name meant to call it is misspelled. The scene still compiles: this rule simply never runs.",
  RESOLVE_VALUE_OUTSIDE_ITS_VOCABULARY: "'{nom}' writes '{cle}:{ecrit}', which the prototype does not list. The example shows the words this member accepts: {mots}. Nothing declares a closed list here \u2014 showing the values IS the declaration, and the first one is the default.",
